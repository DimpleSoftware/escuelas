...
class Tecnico{
	...
	public function listarTecnicoEstado($Tecnico){
		global $docRootSitio;		
		$this->setId($Tecnico);
				
		$q = "SELECT * FROM tecnico WHERE numeroSerie='{$this->getId()}' AND estadoFin='0' LIMIT 1";
	...
	public function listarTecnicos($offset="",$limit="",$campoOrder="",$order=""){		
		global $docRootSitio;		
		
		$q = "SELECT SQL_CALC_FOUND_ROWS * FROM tecnico ";
	...
	$q="INSERT INTO tecnico (id,nombreAlumno,curso,cuil,numeroSerie,marca,problema,fecha,idreclamo,estado,estadoFin) "
			."VALUES ('','{$this->getNombreAlumno()}','{$this->getCurso()}','{$this->getCuil()}','{$this->getNumeroSerie()}','{$this->getMarca()}','{$this->getProblema()}','{$this->getFecha()}','{$this->getIdReclamo()}','{$this->getEstado()}','0');";
