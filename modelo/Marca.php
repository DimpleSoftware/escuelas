...
class Marca{
	...
	public function listarMarca($Marca){
		...
		$q = "SELECT * FROM marca WHERE id='{$this->getId()}' LIMIT 1";
	...
	public function listarMarcas($offset="",$limit="",$campoOrder="",$order=""){		
		...
		$q = "SELECT SQL_CALC_FOUND_ROWS * FROM marca ";
	...
	public function modificarMarca(){
	  ...
	  echo $q="UPDATE marca SET nombre='{$this->getNombre()}' WHERE id='{$this->getId()}'";	
		try{
				$result = mysql_query($q);
					if(!$result) {
						throw new Exception("");
